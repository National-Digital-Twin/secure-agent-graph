...
class TestJenaMetrics {
    ...
    @Test
    void get_meter_02() {
        assertDoesNotThrow(()->JenaMetrics.getMeter("test", Jena.VERSION));
    }
    ...
    private void server_metrics_reader2() throws IOException, InterruptedException {
        ...
        FusekiServer server = SecureAgentGraph.secureAgentGraphBuilder()
                .port(0)
        ...
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .headers(AwsConstants.HEADER_DATA, LibTestsSAG.tokenForUser("test"))
                    .GET()
                    .build();
            ...
            assertTrue(
                reader.collectAllMetrics()
                  .stream()
                  .filter(m -> m.getName().equals("secureagent.graph.requests.good"))
