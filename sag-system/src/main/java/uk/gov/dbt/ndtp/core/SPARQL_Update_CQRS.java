...
public class SPARQL_Update_CQRS extends SPARQL_Update {
    ...
    private static final String UPDATE_PARSE_BASE = Fuseki.BaseParserSPARQL;
    private static final IRIxResolver resolver = IRIxResolver.create()
                                                             .base(UPDATE_PARSE_BASE)
                                                             .resolve(true)
                                                             .allowRelative(false)
                                                             .build();
    ...
    private static final boolean EXECUTE_AS_WRITE = true;

    @Override
    protected void execute(HttpAction action, InputStream input) {
        UsingList usingList = processProtocolUpdateCQRS(action.getRequest());
        if (EXECUTE_AS_WRITE)
            action.beginWrite();
        else
        ...
        try {
            ...
            UpdateAction.parseExecute(usingList, updateCtl.dataset(), input, UPDATE_PARSE_BASE, Syntax.syntaxARQ);
     ...
     * accept them.
     */
    private UsingList processProtocolUpdateCQRS(HttpServletRequest request) {
